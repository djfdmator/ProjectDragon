...
//Animation Legacy Controll Script

using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class AnimatorControll_ver0 : MonoBehaviour
{
    int counter;
    ...
    {
        if (player.AngleisAttack == true||player.isSkillActive)
        {
            temp_name = clearAnimator_name;
            my_state = player.CurrentState;
            anglepos = player.Enemy_AngleCaseString(player.enemy_angle);
            AttackName = "Female_DefaultCloth_"+player.attackType.ToString()+"_" + my_state + "_" + anglepos.ToString();
            ClearAnimator_Name = AttackName;
           // AngleStringCast(AttackName);
        }
        else if (player.AngleisAttack == false)
        {
            temp_name = clearAnimator_name;
            my_state = player.CurrentState;
            anglepos = player.Current_AngleCaseString(player.current_angle);
            ClearAnimator_Name = "Female_DefaultCloth_"+player.attackType.ToString()+ "_" + my_state + "_" + anglepos.ToString();
            //AngleStringCast(clearAnimator_name);
        }
    ...
    private void Awake()
    {
        player = GameObject.Find("테스터").GetComponent<Player>();
        Arm = player.gameObject.transform.GetChild(1).gameObject;
        Weapon = player.gameObject.transform.GetChild(0).gameObject;
    }
    ...
    void Start()
    {
        ...
        {
            my_state++;
            clearAnimator_name = "Female_DefaultCloth_"+player.attackType.ToString()+"_" + my_state.ToString();
            temp_name = clearAnimator_name;
    ...
    void Player_AnimationController_CastingCurrentAnim(AnimationClip[] animationbundle, State state)
    {
        ...
        {
            anglepos++;
            animationbundle[i] = Resources.Load<AnimationClip>("Animation/Player/Female/DefaultCloth/"+ player.attackType.ToString()+"/" + state.ToString() + "/" + temp_name + "_" + anglepos.ToString());
    ...
    void Player_AnimationController_CastingCurrentAnim_Arm(AnimationClip[] animationbundle, State state)
    {
        ...
        {
            anglepos++;
            animationbundle[i] = Resources.Load<AnimationClip>("Animation/Player/Female/DefaultCloth/" + player.attackType.ToString() + "/" + state.ToString() + "/" + temp_name + "_" + anglepos.ToString() + "_Arm");
    ...
    void Player_AnimationController_CastingCurrentAnim_Weapon(AnimationClip[] animationbundle, State state)
    {
        ...
        for (int i = 0; i < animationbundle.Length; i++)
        {
            ...
            anglepos++;
            animationbundle[i] = Resources.Load<AnimationClip>("Animation/Player/Female/DefaultCloth/" + player.attackType.ToString() + "/" + state.ToString() + "/" + temp_name + "_" + anglepos.ToString() + "_Weapon");
