...
public class Encyclopedia_Popup : MonoBehaviour
{
    ...
    {
        #region 무기 슬롯
        List<Database.Encyclopedia> weaponsData = GameManager.Inst.PlayData.collection.encyclopedia_WeaponList;
        ...
        #region 몬스터 슬롯
        List<Database.Encyclopedia> monstersData = GameManager.Inst.PlayData.collection.encyclopedia_MonsterList;
        ...
        #region 업적 슬롯
        List<Database.Achievement> achievementsData = GameManager.Inst.PlayData.collection.achievementList;
    ...
    private void RefreshSlots()
    {
        ...
        {
            cell.ToggleActive(GameManager.Inst.PlayData.collection.encyclopedia_WeaponList[cell.DB_Num].active);
        }
        foreach (Encyclopedia_CellData cell in cellList[Type.Monster])
        {
            cell.ToggleActive(GameManager.Inst.PlayData.collection.encyclopedia_MonsterList[cell.DB_Num].active);
        }
        foreach (Encyclopedia_CellData cell in cellList[Type.Achievement])
        {
            cell.ToggleActive(GameManager.Inst.PlayData.collection.achievementList[cell.DB_Num].active);
        }
    }
    #region 공사중
    private void RefreshSlot(Type type, int DB_Num)
    {
        ...
        {
            GameManager.Inst.PlayData.collection.encyclopedia_WeaponList[DB_Num].active=true;
        }
        else if (type.Equals(Type.Monster))
        {
            GameManager.Inst.PlayData.collection.encyclopedia_MonsterList[DB_Num].active = true;
        }
        else
        {
        }
    ...
    private void SetInfo(Type type, int DB_index)
    {
        ...
        if (type.Equals(Type.Weapon))
        {
            List<Database.Encyclopedia> data = Database.Inst.playData.collection.encyclopedia_WeaponList;
        ...
        {
            List<Database.Encyclopedia> data = Database.Inst.playData.collection.encyclopedia_MonsterList;
            imageName += data[DB_index].imageName;
            strTitle = data[DB_index].name;
            strSubtitle = data[DB_index].description;
        }
        ...
        {
            List<Database.Achievement> data = Database.Inst.playData.collection.achievementList;
            imageName += data[DB_index].imageName;
